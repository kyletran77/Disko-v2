...
import { circlesList, setAllCircles } from "features/allCircle/circleSlice";
import { setAllFeed } from "features/allFeed/allFeedSlice";
import {
  addBookmark,
  login,
  logout,
...
const circleListener = (dispatch) => {
  console.log("circleListener");

  
  try {
    const loader = toast.loading("Loading..");
    const q = query(collection(db, "allCircles"));
    onSnapshot(q, (snapshot) => {
      const allCirclesList = snapshot.docs.map((doc) => doc.data());
      dispatch(setAllCircles(allCirclesList));
    });
    toast.dismiss(loader);
    console.log("Circle Fetched");
  } catch (error) {
    console.error(error.message);
  }
};
...
const feedListener = (dispatch) => {
  try {
    const loader = toast.loading("Loading..");
    const q = query(collection(db, "allFeed"), orderBy("createdAt", "desc"));
    onSnapshot(q, (snapshot) => {
      const allFeedList = snapshot.docs.map((doc) => doc.data());
      dispatch(setAllFeed(allFeedList));
    });
    toast.dismiss(loader);
